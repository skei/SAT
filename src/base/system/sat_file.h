...
SAT_File::SAT_File(const char* AFilename, const char* AMode)
{
...
bool SAT_File::isOpen()
{
    return (MHandle != nullptr);
}

bool SAT_File::exists(const char* AName)
{
...
bool SAT_File::direxists(const char* APath)
{
...
bool SAT_File::open(const char* AFilename, const char* AMode)
{
    MName   = AFilename;
    MMode   = AMode;
...
void SAT_File::close(void)
{
    if (MHandle) fclose(MHandle);
    MHandle = nullptr;
}

uint32_t SAT_File::read(void* ABuffer, uint32_t ALength, uint32_t ASize)
{
    if (MHandle)
    {
...
uint32_t SAT_File::write(void* ABuffer, uint32_t ALength, uint32_t ASize)
{
    if (MHandle)
    {
...
uint32_t SAT_File::write(const char* AText)
{
    if (MHandle)
    {
...
char* SAT_File::readLine(char* ABuffer, uint32_t AMaxSize)
{
