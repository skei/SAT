...
class SAT_ThreadPool
{
    ...
    private:
        void            thread_exec();
        static void*    thread_pool_callback(void* argument);
    private:
...
SAT_ThreadPool::SAT_ThreadPool(SAT_ThreadPoolListener* AListener, uint32_t ANumThreads)
{
    SAT_TRACE;
    MListener = AListener;
...
bool SAT_ThreadPool::request_exec(uint32_t ACount)
{
...
void SAT_ThreadPool::exec_task(uint32_t AIndex)
{
...
void SAT_ThreadPool::thread_exec()
{
    uint32_t index;
    while (true)
    {
...
static void* SAT_ThreadPool::thread_pool_callback(void* argument)
{
