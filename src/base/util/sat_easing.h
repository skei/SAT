...
float SAT_EasingFixTime(float t, float d);
...
float SAT_Easing(uint32_t typ, float t, float b, float c, float d, bool AMirror);
...
float SAT_Easing(uint32_t typ, float t, float b, float c, float d, bool AMirror)
{
    if (AMirror) t = SAT_EasingFixTime(t,d);
    switch (typ)
    {
