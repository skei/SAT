...
class SAT_Color
{
    ...
    public:
        ...
        SAT_Color();
        SAT_Color(sat_color_t v);
        SAT_Color(sat_color_t ar, sat_color_t ag, sat_color_t ab, sat_color_t aa=1.0f);
    ...
    public:
        ...
        void blend(SAT_Color color, sat_color_t alpha);
        void blendA(SAT_Color color, sat_color_t alpha);
...
SAT_Color::SAT_Color(sat_color_t v)
{
    r = v;
    g = v;
    b = v;
...
SAT_Color::SAT_Color(sat_color_t ar, sat_color_t ag, sat_color_t ab, sat_color_t aa)
{
    r = ar;
    g = ag;
    b = ab;
...
SAT_Color& SAT_Color::operator = (uint32_t color)
{
    sat_color_t aa = (color & 0xff000000) >> 24;
    sat_color_t rr = (color & 0x00ff0000) >> 16;
    sat_color_t gg = (color & 0x0000ff00) >> 8;
    sat_color_t bb = (color & 0x000000ff);
    r = rr * SAT_INV255;
    g = gg * SAT_INV255;
    b = bb * SAT_INV255;
    a = aa * SAT_INV255;
...
// bool SAT_Color::operator == (const char* str)
// {
//     return (strcmp(MBuffer,str) == 0) ? true : false;
// }

void SAT_Color::add(SAT_Color color)
{
    r += color.r;
    g += color.g;
    b += color.b;
...
void SAT_Color::sub(SAT_Color color)
{
    r -= color.r;
    g -= color.g;
    b -= color.b;
...
void SAT_Color::mul(SAT_Color color)
{
    r *= color.r;
    g *= color.g;
    b *= color.b;
...
// void SAT_Color::blend(SAT_Color color1, SAT_Color color2, T blend)
// {
//     SAT_Color result;
...
void SAT_Color::blend(SAT_Color color, sat_color_t alpha)
{
...
void SAT_Color::blendA(SAT_Color color, sat_color_t alpha)
{
    r = (color.r * alpha) + (r * (1.0 - alpha));
    g = (color.g * alpha) + (g * (1.0 - alpha));
    b = (color.b * alpha) + (b * (1.0 - alpha));
...
void SAT_Color::blend(SAT_Color color)
{
    blend(color,color.a);
}
