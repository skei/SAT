...
void SAT_CallStack::activate(SAT_GlobalBase* AGlobal)
{
    if (!MIsActivated)
    {
        MGlobal = AGlobal;
...
void SAT_CallStack::deactivate(SAT_GlobalBase* AGlobal)
{
    if (MIsActivated)
    {
        MGlobal = nullptr;
...
/*
    ...
    the first entries are usually
        0: SAT_CallStack::print(int)
        1: SAT_CrashHandler::crashHandler(int)
        2: sat_crash_handler_callback(int)
        3: (null)
        4: the place it actually crashes..
    
    (3 is probably some kernel or signal handler thing)
    so the first four is normally not very interesting, so we can skop them
    which is why there's a skip=4 default argument to print()..
*/

void SAT_CallStack::print(int skip)
{
    #if defined (SAT_DEBUG_CALLSTACK)
        SAT_GLOBAL_DPRINT("Callstack:\n");
        void* callstack[128];
        ...
        if (skip > 0)
        {
            SAT_GLOBAL_DPRINT("  (skipping first %i entries)\n",skip);
        }
