...
SAT_GlobalPrint::SAT_GlobalPrint()
{
    #ifdef SAT_PRINT_SOCKET
        setupSocket();
    #endif
...
void SAT_GlobalPrint::activate(SAT_GlobalBase* AGlobal)
{
    if (!MIsActivated)
    {
        MGlobal = AGlobal;
...
void SAT_GlobalPrint::deactivate(SAT_GlobalBase* AGlobal)
{
    if (MIsActivated)
    {
        MGlobal = nullptr;
...
void SAT_GlobalPrint::clearPrefix()
{
    MPrefixBuffer[0] = 0;
}

void SAT_GlobalPrint::setPrefix(const char* AFile, const char* AFunc, uint32_t ALine)
{
...
void SAT_GlobalPrint::print(const char* format, ...)
{
    va_list args;
...
const char* SAT_GlobalPrint::stripPath(const char* buffer)
{
    const char *slash;
...
//------------------------------
    ...
    void SAT_GlobalPrint::cleanupSocket()
    {
        #if defined SAT_LINUX
            close(MSocketHandle);
        #elif defined SAT_WIN32
    ...
    void SAT_GlobalPrint::print_socket(const char* fmt, const char* pre, const char* str)
    {
        #if defined SAT_LINUX
            //dprintf(MSocketHandle,"%s%s",MPrefixBuffer,MPrintBuffer);
            dprintf(MSocketHandle,fmt,pre,str);
        #else
            #warning NOT IMPLEMENTED YET (printing to stdout)
            //TODO (print socket in windows)
            printf(fmt,pre,str);
        #endif
    }

#endif // SAT_PRINT_SOCKET
...
//------------------------------
    ...
    void SAT_GlobalPrint::timeStart()
    {
        struct timeval time;
    ...
    double SAT_GlobalPrint::timeElapsed()
    {
        struct timeval time;
        gettimeofday(&time,NULL);
