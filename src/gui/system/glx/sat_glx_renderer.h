...
#include "gui/system/glx/sat_glx.h"
...
SAT_GlxRenderer::~SAT_GlxRenderer() {
...
bool SAT_GlxRenderer::makeCurrent()
{
...
bool SAT_GlxRenderer::resetCurrent()
{
    //SAT_TRACE;
    //SAT_PRINT("MIsCurrent: %i\n",MIsCurrent);
...
bool SAT_GlxRenderer::beginRendering()
{
    SAT_TRACE;
    makeCurrent();
    //SAT_TRACE;
...
bool SAT_GlxRenderer::beginRendering(int32_t AWidth, int32_t AHeight)
{
    makeCurrent();
    //SAT_TRACE;
...
bool SAT_GlxRenderer::endRendering()
{
    swapBuffers();
    //resetClip();
    resetCurrent();
...
bool SAT_GlxRenderer::setViewport(int32_t AXpos, int32_t AYpos, int32_t AWidth, int32_t AHeight)
{
    glViewport(AXpos,AYpos,AWidth,AHeight);
    return true;
}

bool SAT_GlxRenderer::swapBuffers()
{
    //SAT_TRACE;
...
bool SAT_GlxRenderer::enableVSync()
{
...
bool SAT_GlxRenderer::disableVSync()
{
...
GLXFBConfig SAT_GlxRenderer::findFBConfig(const int* AAttribs)
{
    //SAT_PRINT;
    int num_fbc = 0;
    //SAT_PRINT("Display: %p\n",MDisplay);
...
GLXContext SAT_GlxRenderer::createContext(GLXFBConfig fbconfig)
{
...
void SAT_GlxRenderer::destroyContext()
{
    unloadExtensions();
...
GLXContext SAT_GlxRenderer::getCurrentContext()
{
    return glXGetCurrentContext();
}  

bool SAT_GlxRenderer::getGLXVersion(int* AMajor, int* AMinor)
{
    //SAT_PRINT;
...
/*
    bool SAT_GlxRenderer::checkVersion(Display* display, int major, int minor)
    {
...
Display* SAT_GlxRenderer::getCurrentDisplay()
{
    //SAT_PRINT;
...
GLXDrawable SAT_GlxRenderer::getCurrentDrawable()
{
    //SAT_PRINT;
    return glXGetCurrentDrawable();
}

GLXDrawable SAT_GlxRenderer::getCurrentReadDrawable()
{
    //SAT_PRINT;
...
bool SAT_GlxRenderer::isExtensionSupported(const char *extList, const char *extension)
{
    const char *start;
...
bool SAT_GlxRenderer::loadExtensions()
{
...
void SAT_GlxRenderer::unloadExtensions()
{
    sogl_cleanup();
}
