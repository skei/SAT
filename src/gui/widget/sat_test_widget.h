...
: public SAT_TextWidget
{
    ...
    public:
        ...
        // void on_widget_mouse_start_drag(int32_t AXpos, int32_t AYpos, uint32_t AButton, uint32_t AState, uint32_t ATime) override;
...
void SAT_TestWidget::on_widget_mouse_event(uint32_t AEvent, SAT_MouseState* AState)
{
    const char* name = getName();
    SAT_PRINT("%s : event: %i state: %i\n",name,AEvent,AState->id());
}

void SAT_TestWidget::on_widget_mouse_gesture(uint32_t AGesture, SAT_MouseState* AState)
{
    const char* name = getName();
    SAT_PRINT("%s : gesture: %i state: %i\n",name,AGesture,AState->id());
}

void SAT_TestWidget::on_widget_keyboard_event(uint32_t AEvent, SAT_KeyboardState* AState)
{
    const char* name = getName();
    SAT_PRINT("%s : event: %i state: %i\n",name,AEvent,AState->id());
}

void SAT_TestWidget::on_widget_keyboard_gesture(uint32_t AGesture, SAT_KeyboardState* AState)
{
    const char* name = getName();
    SAT_PRINT("%s : gesture: %i state: %i\n",name,AGesture,AState->id());
}
