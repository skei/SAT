...
typedef SAT_SPSCQueue<SAT_TweenChain*,SAT_QUEUE_SIZE_TWEEN>  SAT_TweenChainQueue;
...
SAT_TweenNode::SAT_TweenNode(void* ATarget, uint32_t AId, double ADuration, uint32_t AType, uint32_t ANumValues, double* AStartValues, double* AEndValues, uint32_t ATween)
{
    //SAT_TRACE;
    MId           = AId;
    MDuration     = ADuration;
...
SAT_TweenNode* SAT_TweenChain::appendNode(SAT_TweenNode* ANode)
{
    MNodes.append(ANode);
    return ANode;
}

void SAT_TweenChain::deleteNodes()
{
    for (uint32_t i=0; i<MNodes.size(); i++) delete MNodes[i];
}

uint32_t SAT_TweenChain::getNumNodes()
{
    return MNodes.size();
}
  
void SAT_TweenChain::start(void)
{
    MActive = true;
    MCurrentNode = 0;
    MCurrentTime = 0.0;
...
void SAT_TweenChain::stop(void)
{
    MActive = false;
}
  
bool SAT_TweenChain::isActive()
{
    return MActive;
}
...
SAT_TweenManager::SAT_TweenManager()
{
}
...
// void SAT_TweenManager::appendTween(SAT_TweenNode* ATween)
// {
//     // create chain
//     // append node
// }
  
SAT_TweenChain* SAT_TweenManager::appendChain(SAT_TweenChain* AChain)
{
    MPending.write(AChain);
    return AChain;
}
  
void SAT_TweenManager::deleteChains()
{
...
void SAT_TweenManager::process(double ADelta)
{
    // SAT_TRACE;
